...
| M. 合成ユーザーリサーチ | 0 | 申告あり（チームへの聞き取り：四半期ごとのユーザーインタビュー） | 該当する Issue・ラベルが 0 件 | AI でユーザー視点を試した記録が 1 件あれば 1 |
| N. 継続的改善のフィードバックループ | 0 | 申告あり（チームへの聞き取り：毎スプリント、オンラインホワイトボードで実施） | ふりかえりの文書・PR が 0 件（GitHub の外で実施、後述） | ふりかえりの結果を GitHub に残せば 1〜2 |
...
## 別の例：ライブラリ・GitHub Action の場合（判定基準シート 0.2.0）

> **これも架空のリポジトリ `example-org/lint-action`（静的解析を実行する GitHub Action。メンテナー 1 名）を想定した抜粋です。** 本番環境を持たないリポジトリで、`criteria.md` の「N/A 注記（リポジトリの種類）」をどう当てるかだけを示します。

リポジトリの種類は「ライブラリ・GitHub Action・CLI」と判断しました（根拠：ルートに `action.yml` があり、デプロイ用のワークフローは Marketplace への公開だけ。データを取り込んで保持する処理は無い）。

| 項目 | スコア | 理由 |
| --- | --- | --- |
| E. テスト・QA | 0 | | テストはワークフローで毎回実行しているが、AI でテストを評価・生成した記録が無い。テストの自動化の状況は「所見」に書く |
| G. 監視・インシデント対応 | N/A | | 自分で運用する本番環境が無い |
| H. データ管理 | N/A | | データを取り込み・保持しない |
| M. 合成ユーザーリサーチ | N/A | 申告あり（チームへの聞き取り：四半期ごとのユーザーインタビュー） | 利用者向けの UI や対話面が無い（入力はワークフローの `with:` だけ） |

| 指標 | 値 |
| --- | --- |
| 全項目の中央値（平均） | **1**（1.2）。G・H・M（N/A）を除く 13 項目 |
| 階層準拠（全項目の最小値） | **0**（E. テスト・QA ほか）。G・H・M（N/A）を除く |

N/A の項目は、軸スコア・中央値・平均・最小値のどれにも入れません。E のように AI の基準を満たさない項目は、AI 以外の自動化が充実していてもスコアを上げず、所見に書きます。
